...
export function useSmartRecipes() {
    ...
    const scoreRecipe = (recipe) => {
        ...
        // 檢查是否符合月齡
        const isAgeAppropriate = babyAgeMonths >= recipe.min_month && babyAgeMonths <= recipe.max_month;
        // 檢查過敏食材
     ...
     * 取得符合月齡的食譜
     */
    const ageAppropriateRecipes = computed(() => {
     ...
     * 取得可立即製作的食譜
     */
    const readyToCookRecipes = computed(() => {
    ...
    const recommendedRecipes = computed(() => {
        return scoredRecipes.value.filter((sr) => sr.isAgeAppropriate && !sr.hasAllergyIngredients);
    });
    return {
        scoredRecipes,
...
//# sourceMappingURL=useSmartRecipes.js.map